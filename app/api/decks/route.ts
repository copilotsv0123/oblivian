...
export const GET = withApiHandler(async ({ user, request }: ApiContext) => {
  ...
  if (Object.keys(filters).length > 0) {
    const filteredDecks = await deckRepository.findByFilters(
      user.id,
      filters,
      true,
    );
  ...
  const dashboardDecks = await deckRepository.findForDashboardWithGrades(
    user.id,
  );
...
export const POST = withApiHandler(async ({ user, request }: ApiContext) => {
  const { title, description, level, language, isPublic, tags } =
